...
type TaskPrivateData struct {
	...
	// LastFrameURL 上游返回的尾帧图地址（创建任务时带 return_last_frame=true 才有）。
	// 存下来是为了在轮询结束后仍能对外提供——和 ResultURL 同理，不存就丢了。
...
// GetTaskByIdGlobal 按 task_id 查询任意用户的任务（管理员查看视频用）。
func GetTaskByIdGlobal(taskId string) (*Task, bool, error) {
	if taskId == "" {
		return nil, false, nil
	}
	var task *Task
