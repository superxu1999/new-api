...
界面上的显示规则：侧边栏里的素材库模块由系统设置的侧边栏配置控制；账号开关只控制模块内的功能——「素材库」决定该账号能否使用素材（素材组与素材入库），「上传素材」决定能否直传本地文件。真人认证不受这两个开关限制，任何已登录账号都可以完成。

素材接口（`/v1/assets`、`/v1/assets/groups`）**默认按账号关闭**，需要超级管理员开通后才能调用；未开通时返回 403 `asset_library_disabled`。真人认证接口（`/v1/assets/real-person/sessions`）不受该开关限制。
