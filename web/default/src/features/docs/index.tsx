              ...
              <Code>{`HTTP/1.1 200 OK
{
          ...
          <Section id='sec-7' title={t('7. 素材库')}>
            ...
            <Sub id='sec-7-1' title={t('7.1 权限与准备')}>
              <T
                headers={['项目', '说明']}
                rows={[
                  ['素材接口', '需开通「素材库」权限；未开通时返回 403 asset_library_disabled'],
                  ['上传素材接口', '需开通「上传素材」权限；未开通时返回 403 asset_upload_disabled'],
            ...
            <Sub id='sec-7-2' title={t('7.2 素材组：列表 / 新建 / 重命名 / 删除')}>
              ...
              <P>
                {t('返回当前账号的素材组。真人素材组由真人认证流程自动生成，不能通过该接口创建。')}
              </P>
              <ET title={t('请求参数')} />
              <T
                headers={['参数', '类型', '必填', '说明']}
  ...
  -H "Authorization: Bearer sk-..."`}</Code>
            ...
            <Sub id='sec-7-3' title={t('7.3 素材：新建（公网 URL）')}>
              ...
              <Code>{`HTTP/1.1 200 OK
{
  ...
  "data": {
    ...
    "updated_at": 1790047202
            ...
            <Sub id='sec-7-4' title={t('7.4 素材：查询列表')}>
              <Endpoint method='GET' path='/v1/assets' />
              <P>
                {t('返回当前账号的素材列表，状态为最近一次同步结果。')}
              </P>
              <ET title={t('请求参数')} />
              <T
              ...
              <Code>{`HTTP/1.1 200 OK
{
  ...
  "page_size": 20
            ...
            <Sub id='sec-7-5' title={t('7.5 素材：详情 / 重命名 / 删除')}>
  ...
  -H "Authorization: Bearer sk-..."`}</Code>
            ...
            <Sub id='sec-7-6' title={t('7.6 上传素材')}>
              ...
              <P>
                {t('上传本地文件：文件由本站暂存并提供下载地址，供素材库读取后完成入库。需开通「上传素材」权限；未开通时返回 403 asset_upload_disabled。')}
              </P>
  ...
  -F "name=角色定妆图"`}</Code>
            ...
            <Sub id='sec-7-7' title={t('7.7 在生成请求中引用素材')}>
              <Endpoint method='POST' path='/v1/videos' />
              <P>
                {t('在生成请求的媒体字段中填写 asset://<本站素材 ID>；系统校验素材归属与状态后再提交生成。')}
              </P>
              <ET title={t('引用写法')} />
              <T
                headers={['位置', '写法']}
