...
// Doubao 视频支持参考视频输入（metadata.content 中的 video_url）。
...
// CancelTask 取消（删除）上游任务：火山方舟原生接口为
//
//	DELETE {base}/api/v3/contents/generations/tasks/{task_id}
//
// 与 FetchTask 使用同一路径前缀，因此 ARK 直连（火山原生）渠道可直接复用。
...
func (a *TaskAdaptor) convertToRequestPayload(req *relaycommon.TaskSubmitReq) (*requestPayload, error) {
	...
	// 参考素材（顶层 content / images / metadata 扁平写法）已在校验阶段由
	// relaycommon.normalizeTaskReferences 统一归一成 metadata.content，这里只读它。
