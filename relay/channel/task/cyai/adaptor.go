...
func (a *TaskAdaptor) BuildRequestBody(c *gin.Context, info *relaycommon.RelayInfo) (io.Reader, error) {
	...
	// 顶层 content / images / 扁平写法都已在校验阶段归一到它）时统一把 prompt 作为
	// content[0] 的 text，并把图/视频/音频参考追加进去；
...
//
// 参考素材的收集（顶层 content / images / metadata 扁平写法）统一由校验阶段的
// relaycommon.normalizeTaskReferences 完成，这里只读归一后的 metadata.content。
